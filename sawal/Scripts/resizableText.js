...
function setFontSize(word, display, minFontSize){
   $("#testArea").css("font-family", $(display).css('font-family'));    
    ...
    var displayWidth = parseInt($(display).css('width'))+50;    
    ...
    else if (getWordSize(word, fontSize) < displayWidth+25)
    ...
    if(display=="#tech0"){
        ...
        if(fontSize-1<minFontSize){
            ...
            $(display).css("font-size", fontSize);
            if(word!=undefined) {
                var strWord = abbreviateWord(display, word);
                $(display).text(strWord);
            }
        }
        else{
            $(display).css("font-size", fontSize);    
